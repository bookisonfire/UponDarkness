...
def save_game_data(player_name, scene, decisions, chains, previous_scene):
    game_data = {
    ...
    if os.path.exists("game_data.json"):
        try:
            with open("game_data.json", "r") as file:
                ...
                return game_data["player_name"], game_data["scene"], game_data["previous_scene"], game_data["decisions"], game_data ["chains"]
...
def choose(prompt, options, state):
    ...
    for i, option in enumerate(options, 1): #give each option a number
        type(f"{i}. {option}")
    print("Enter [q]Quit [s]Save Game")
    choice = input("> ")
    if choice.lower() == "s":
        save_game_data(state["player_name"], state["scene"], state["decisions"], state["chains"], state["previous_scene"])
        return choose(prompt, options, state)
    if choice.lower() == "l":
        player_name, scene, previous_scene, decisions, chains = load_game_data()
        if (player_name is not None and scene is not None and previous_scene is not None and decisions is not None and chains is not None):
            state["player_name"] = player_name
            state["scene"] = scene
            state["decisions"] = decisions if isinstance(decisions, list) else []
            state["chains"] = chains
            state["previous_scene"] = previous_scene
            return choose(prompt, options, state)
        else:
            print("No saved game data found.")
            return choose(prompt, options, state)
    if choice.lower() == "q" or choice.lower() == "esc":
        return "quit"
    if choice.isdigit() and 1 <= int(choice) <= len(options):
        ...
        print("Invalid choice. Please select a number between 1 and", len(options))
...
def main():
    ...
    if saved_state:
        ...
        if choice == "y":
            player_name, scene, previous_scene, chains, decisions = saved_state
            state = {"player_name": player_name, "scene": scene, "previous_scene": previous_scene, "chains": chains, "decisions": decisions}
        else:
            state = {"player_name": None, "scene": None, "previous_scene": None, "chains": False, "decisions": []}
            player_name = get_player_name(state)
            state["player_name"] = player_name
    else:
        state = {"player_name": None, "scene": None, "previous_scene": None, "chains": False, "decisions": []}
        player_name = get_player_name(state)
